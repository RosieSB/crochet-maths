...
:::{figure} figs/7-colour-1.jpg
:width: 400
:label: fig:7-colour-1
Measured w and h do not match pattern parameters
:::


### Attempt 2

Generate new pattern with measured parameters from first attempt.
...
Attempt 2 parameters: R={eval}`R`, r={eval}`r`, w={eval}`w`, h={eval}`h`.
...
Attempt 3 parameters: R={eval}`R`, r={eval}`r`, w={eval}`w`, h={eval}`h`. 
...
fig = plt.figure(figsize = (8,4),label=' ',layout='tight')
ax = plt.axes()
marker-size = rcParams['lines.markersize']

for n in range(N+1):
    for k in range(st_count[n]):
        # upper
        ax.scatter(k*2*np.pi/(st_count[n]-1), n, color=colours_upper[n][k], s=marker-size)
        #lower
        if 0<n<N:
            ax.scatter(k*2*np.pi/(st_count[n]-1), -n, color=colours_lower[n][k])
