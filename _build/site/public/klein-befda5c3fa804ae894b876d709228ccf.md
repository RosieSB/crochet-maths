...
for k in range(i,j+1):
    ...
    #print('\n')
    #instructions = 'Leave a gap '+str(st_omit)+' dc wide, while increasing by '+str(increase)+'dc evenly throughout the rest of the row.
    si_data[k-i] = np.array([row,st_omit,increase, st_count])

si_data = [int(s) for s in si_data]
si_table = tabulate(si_data, headers=['Row','Stitches to omit','Increases','Stitch count (includes missing stitches)'],tablefmt='html')
:::

**Row {eval}`int(si_data[0,0])`:** Omit {eval}`si_data[0,1]` stitches from the previous row, placing the gap so as to line up with the handle as it passes through the main body section. Work the rest of the round as previous, increasing {eval}`si_data[0,2]` evenly throughout ({eval}`si_data[0,3]`-{eval}`si_data[0,1]` sts).
