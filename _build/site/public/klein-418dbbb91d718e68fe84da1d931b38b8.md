...
:::{admonition} Example
For the pattern example in [](#patt_eg), the parameter values were H={eval}`H`, a={eval}`a`, b={eval}`b`, d={eval}`np.round(d,dp)`, and $\theta=${eval}`theta`. So $P=${eval}`P`, $Q=${eval}`Q`, and $\Vert\overrightarrow{PQ}\Vert=${eval}`magPQ`.
:::
