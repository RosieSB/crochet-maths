...
Moderately careful measurement suggests that for [](#fig:labelled-stitch) specifically, $a=${eval}`a`, $b=${eval}`b` and $h=${eval}`h`. Substituting these into [](#eq:theta-n) gives $\theta =${eval}`theta` and $n=${eval}`n`
