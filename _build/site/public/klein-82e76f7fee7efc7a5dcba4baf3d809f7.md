...
We will see that, for an adequate choice of $d$, these two parametrisations agree along the boundary $u=d$ (note $\dot{r}=0$ here, and the Frenet-Serret apparatus $\mathbf{T}$, $\mathbf{N}$, $\mathbf{B}$ are the standard basis vectors $e_3$, $e_2$, $e_1$, respectively. See [](#canal_v_SoR) for more detailed discussion of these two methods for generating parametrised surfaces from curves.
