...
:::
## Derivation
The majority of the Klein bottle pattern comes from a canal surface parametrisation with the following directrix and radius function.
...
We will see that these two parametrisations agree along the boundary $u=d$, since $\dot{r}=0$ here, and the Frenet-Serret apparatus $\mathbf{T}$, $\mathbf{N}$, $\mathbf{B}$ are the standard basis vectors $e_3$, $e_2$, $e_1$, respectively. See [](#canal_v_SoR) for more detailed discussion of these two methods for generating parametrised surfaces from curves.
...
Hence $L$ has Cartesian equation $z=(\cot\theta)y+d+b(\csc\theta-\cot\theta)$, subject to [](#hab-constraint).
...
- The parameter values were $H =$ {eval}`float(np.round(H,dp))`, $a =$ {eval}`a`, $b =$ {eval}`b`, $d =$ {eval}`float(np.round(d,dp))`, and $\theta =$ {eval}`theta`. 
- $P = (0,$ {eval}`float(np.round(P[1],dp))` $,$ {eval}`float(np.round(P[2],dp))` $)$, $Q = (0,$ {eval}`float(np.round(Q[1],dp))` $,$ {eval}`float(np.round(Q[2],dp))` $)$, $\Vert\overrightarrow{PQ}\Vert =$ {eval}`float(np.round(magPQ,dp))`.
- $T =$ {eval}`float(np.round(t[7],dp))`

:::{code-cell} python
:tags: remove-input
fig, directrix = plt.subplots(1, 1, figsize=(4, 6))
directrix.set_aspect('equal')
directrix.set_xlabel('y')
directrix.set_ylabel('z')

directrix.plot(gamma1[1],gamma1[2],color='tab:blue')
directrix.text(Q[1]/2,(d+Q[2])/2,'γ₁',color='tab:blue')

directrix.plot(gamma2[1],gamma2[2],color='tab:orange')
directrix.text((P[1]+Q[1])/2+.2,(P[2]+Q[2])/2-.2,'γ₂',color='tab:orange')

directrix.plot(gamma3[1],gamma3[2],color='tab:green')
directrix.text(a,H+a-.4,'γ₃',color='tab:green')

directrix.plot(gamma4[1],gamma4[2],color='tab:purple')
directrix.plot(gamma5[1],gamma5[2],color='tab:purple')
directrix.plot(gamma6[1],gamma6[2],color='tab:purple')
directrix.text(.1,(H+d)/2,'γ₄',color='tab:purple')
...
### Matplotlib model
We can now apply what we've learnt to equation [](eq:canal-unit-speed) and plot the result.
...
Adding the base (equation [](eq:base-SoR)), we have:
:::{code-cell} python
:tags: hide-input
fig, ax = plt.subplots(1, 1, figsize=(4, 6), subplot_kw={'projection': '3d'})
ax.set_title('Klein bottle with base')
