...
- Eqn. B: {eval}`float(np.round(r1const*sqrt1mt2,dp))` $+$ {eval}`float(np.round(tanphi*sqrt1mt2,dp))` $u_1 = $ {eval}`float(np.round(gamma2const[1],dp))` $+$ {eval}`float(np.round(-PQhat[1],dp))`$u_2\mp$ {eval}`float(np.round(-p*PQhat[2],dp))`

- Eqn. C: {eval}`float(np.round(gamma13const+r1const*tanphi,dp))` $-$ {eval}`float(np.round(1-tanphi**2,dp))`$u_1 = $ {eval}`float(np.round(gamma2const[2],dp))` $+$ {eval}`float(np.round(-PQhat[2],dp))`$u_2\pm$ {eval}`float(np.round(-p*PQhat[1],dp))`
