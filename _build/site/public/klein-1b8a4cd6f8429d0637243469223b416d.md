...
8. {eval}`dual_st_count_b[0]` dc in each st around, {eval}`int(np.round(magPQ/h))` times (straight section of pipe)
9. Work {eval}``dual_st_count_a[0]` dc around. 
10. Work a **short row** of {eval}`dual_st_count_a[1]` stitches into the next round.
