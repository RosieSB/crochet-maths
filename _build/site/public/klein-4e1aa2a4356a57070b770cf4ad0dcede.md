...
si_table = tabulate(si_data, headers=['Row','Stitches to omit','Increases','Stitch count (includes omitted stitches)'],tablefmt='html')
:::
...
**Rows {eval}`int(si_data[2,0])`-{eval}`int(max(si_data[:,0]))`:** Use the summary table below to continue working in this way, leaving a gap of specified width and working increases evenly around each row.
:::{code-cell}
:tags: remove-input
