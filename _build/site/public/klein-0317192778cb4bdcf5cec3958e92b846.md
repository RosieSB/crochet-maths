...
**Row {eval}`si_data[0,0]`:** Omit {eval}`si_data[0,1]` stitches from the previous row, placing the gap so as to line up with the handle as it passes through the main body section. Work the rest of the round as previous, increasing {eval}`si_data[0,2]` evenly throughout ({eval}`si_data[0,3]`-{eval}`si_data[0,1]` sts).
