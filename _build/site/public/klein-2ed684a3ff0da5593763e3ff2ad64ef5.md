...
Base: 
- Stretched half-torus, cross section half-ellipse with height $d=${eval}`float(np.round(d,dp))` and width $\frac{w}{2}-p=${eval}`float(np.round(w/2-p,dp))`  
...
&= \left(\begin{array}{c} r\sqrt{1-\dot r^2}\sin(v) \\ r\sqrt{1-\dot r^2}\cos(v) \\ u-r\dot r\end{array}\right)
\end{align*}
