...
Edu^2+2Fdudv+Gdv^2
$$
associated with the parametrised surface.  In fact, $F=0$ for each parametrisation (another example of a Clairaut surface). Moreover, $E:=\mathbf{x}_u^2$ determines row count, and $G:=\mathbf{x}_v^2$ determines stitch count per row.See [](#t-c) for more details.
