...
Total height: H+a+p={eval}`float(np.round(H+a+p,dp))`
...
-\tan\phi & \text{ for } d+e\sin\phi\leq u\leq H-c\sin\phi \\
...
Still working with stitch dimension w={eval}`w`, h={eval}`h`, work a curved pipe formed of {eval}`int(st_count[max(row)-1])` {eval}`int(np.round(2*np.pi*p/w))` stitches using the technique described in [](#pipe). 

For the top part of the handle, use radii R=a={eval}`a` and r=p={eval}`p`, repeating the rows of [](#Ex1) until you have turned a total angle of {eval}`np.pi+theta` radians.

Then, work a straight section consisting of {eval}`int(np.round(magPQ/h))` rows.

Finally, work another curved pipe, this time with radii R=b={eval}`b` and r=p={eval}`p`, turning a total of {eval}`theta` radians. 
