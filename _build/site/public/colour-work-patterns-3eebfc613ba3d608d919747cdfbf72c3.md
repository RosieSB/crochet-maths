...
$$
\gamma(g) = \lfloor\frac{7+\sqrt{1+48g}}{2}\rfloor.
$$
...
# Set up different viewing directions
axs["A"].view_init(elev=30, azim=45, roll=0)
axs["B"].view_init(elev=30, azim=225, roll=0)
axs["C"].view_init(elev=-30, azim=45, roll=0)
axs["D"].view_init(elev=-30, azim=225, roll=0)
...
for n in range(int(N)+1):
    ...
    for k in range(0,st_count[n]):
        ...
        for p in ["A","B","C","D"]:
            axs[p].plot_surface(x, y, z, color=colours[n][k], edgecolor='black', linewidth=lw)
        axs["E"].scatter(k-st_count[n]/2, n, color=colours[n][k])
...
for p in ["A","B","C","D"]:
    ...
    axs[p].set_ylabel('y')
    axs[p].set_zlabel('z')
    axs[p].set(zlim=(-R-r,R+r))
    axs[p].set_aspect('equal')

# Subfigure E is tapestry crochet chart
axs["E"].set_xlabel('Stitch')
axs["E"].set_ylabel('Row')
