...
for n in range(N+1):
    for k in range(st_count[n]):
        # upper
        ax.scatter(k-st_count[n]/2, n, color=colours_upper[n][k])
        #lower
        if 0<n<N:
            ax.scatter(k-st_count[n]/2, -n, color=colours_lower[n][k])
        
# Axis labels 
ax.set_xlabel('Stitch')
ax.set_ylabel('Row')
...
ls=plt.colors.LightSource(azdeg=315, altdeg=45, hsv_min_val=0, hsv_max_val=1, hsv_min_sat=1, hsv_max_sat=0)
...
for n in range(N+1):
    for k in range(st_count[n]):
        ...
        if n==0:
            ...
            lw=.1
        ax.plot_surface(x, y, z, color=colours_upper[n][k], edgecolor='black', linewidth=lw, lightsource=ls)
        #lower
        if 0<n<N:
            u = np.linspace((-n-.5)*np.pi/N, (-n+.5)*np.pi/N, 2)
            v = np.linspace((k-.5)*2*np.pi/st_count[n], (k+.5)*2*np.pi/st_count[n], 2)
            ...
            y = (R-r*np.cos(u))*np.sin(v)
            z = r*np.sin(u)
            ax.plot_surface(x, y, z, color=colours_lower[n][k], edgecolor='black', linewidth=lw, lightsource=ls)
