...
$$
\gamma_2 = \frac{-d-b\theta}{\Vert\overrightarrow{PQ}\Vert}P + \frac{1+d+b\theta}{\Vert\overrightarrow{PQ}\Vert}Q - u\hat\overrightarrow{PQ}
$$
...
r_1(u_1)\sqrt{1-\tan^2\phi}\cos(v_1) &=& (\gamma_2(u_2))_2+p\hat{\overrightarrow{PQ}}_3 &\text{ (B)}\\
(\gamma_1(u_1))_3+r_1\tan\phi &=& (\gamma_2(u_2))_3-p\hat{\overrightarrow{PQ}}_2 &\text{ (C)}
\end{array}\right.
$$
...
- Eqn. B: $\big($ {eval}`float(np.round(r1const*sqrt1mt2,dp))` $+$ {eval}`float(np.round(tanphi*sqrt1mt2,dp))` $u_1\big)\cos(v_1) = $ {eval}`float(np.round(p,dp))` $\sin(v_2)$
