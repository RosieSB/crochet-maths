...
#Curvatures
kappa = float(np.round(1/R,3))
K_inner = float(np.round(-1/(r*(R-r)),3))
K_outer = float(np.round(1/(r*(R+r)),3))


#Turning angle
#np.set_printoptions(legacy='1.25')
theta = float(np.round(2*np.arcsin(w/(2*(R-r))),3))
:::
...
d = -1 #widgets.FloatSlider(value=-1,min=-5,max=0,step=.1,description='d')
a = 1 #widgets.FloatSlider(value=1,min=0,max=5,step=.1,description='a')
b = 2 #widgets.FloatSlider(value=2,min=0,max=5,step=.1,description='b')
theta = 1 #widgets.FloatSlider(value=1,min=0,max=np.pi/2,step=.1,description='θ')
...
ax.text(0, -.3*a ,h/2 ,r'$\gamma_1$', color='tab:blue', fontsize=16)
...
ax.text(0, 2.1*a ,1.1*h ,r'$\gamma_2$', color='tab:orange', fontsize=16)
...
ax.text(0, (P[1]+Q[1])/2-.1*a ,(P[2]+Q[2])/2-.1*a ,r'$\gamma_3$', color='tab:green', fontsize=16)
...
b=float(np.round(b,3))
d=float(np.round(d,3))
h=float(np.round(h,3))

if b<=a:
    ax.set_yticks([0,b,a], labels=["0", str(b), str(a)])
else:
    ax.set_yticks([0,b,a], labels=["0", str(a), str(b)])

ax.set_zticks([d, 0,h,h+a], labels=[str(d), "0", str(h), str(h+a)])
...
Directrix experiment, with d={eval}`d`, h={eval}`float(np.round(h,3))`, a={eval}`a`, b={eval}`b`
::::