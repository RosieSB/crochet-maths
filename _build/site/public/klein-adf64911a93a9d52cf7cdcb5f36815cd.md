...
**Rows {eval}`rc[5]+1`–end:**
Work the following row counts in the round, spacing increases and decreases evenly throughout each round.
...
#canal
u = np.concatenate([usplit[4][1:],usplit[5][1:],usplit[6][1:]])
r = np.concatenate([r4[1:],r5[1:],r5[1:]])
rdot = np.concatenate([r4dot[1:],r5dot[1:],r6dot[1:]])
rddot = np.concatenate([r4ddot[1:],r5ddot[1:],r6ddot[1:]])
z = np.zeros(l)
z = u-r*rdot
...
#add base
u = np.concatenate([usplit[7],u])
z = np.concatenate([d+d*np.sin(u7),z])
E = np.concatenate([E7,E])
G = np.concatenate([G7,G])
du = np.concatenate([du7,du])
...
data = np.transpose([row, st_count])
