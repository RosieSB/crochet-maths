...
**Row 1:** {eval}`int(base7[0,1])` dc into foundation chain.

**Rows 2-{eval}`max(row)`:**  Work the following stitch counts evenly throughout each round to form the inside of the base/very bottom of the handle:
:::{code-cell} python
:label: handle-table
:tags: remove-input
