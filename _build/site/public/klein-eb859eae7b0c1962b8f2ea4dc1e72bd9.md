...
usplit = [np.linspace(-np.pi/2, 0, n),   #t[0], t[1], n),
...
v = np.linspace(0,2*np.pi,100)
u, v = np.meshgrid(u, v)
r0 = np.zeros(len(usplit[0]))
r0dot = np.zeros(len(usplit[0]))
r0ddot = np.zeros(len(usplit[0]))
x = ((e+p)/2+((e-p)/2)*np.cos(u))*np.sin(v)
y = ((e+p)/2+((e-p)/2)*np.cos(u))*np.cos(v)
z0 = d+d*np.sin(u)
E0 = ( (e-p)*np.sin(u)/2 )**2 + ( d*np.cos(u) )**2
ax.plot_surface(x, y, z0, edgecolor = 'none', linewidth = .1, alpha = .25)
...
table = tabulate(data,headers=['u','z','E','sqrtE','du','dz','ds','s','Circumference','Row','Stitch count'],tablefmt='html')
