...
for k in range(i,j+1):
    ...
    si_data[k-i] = np.array([row, instructions, st_count])
    
    display(HTML(' </bf> Row  str(row) <bf> '))
si_table = tabulate(si_data, headers=['Row', 'Instructions', 'Stitch count (includes missing stitches)','Increase','Stitches to omit from previous row', 'u'],tablefmt='html')
display(si_table)
