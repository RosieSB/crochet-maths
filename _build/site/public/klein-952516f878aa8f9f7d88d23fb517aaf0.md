...
Stitch width and stitch count per row will behave more regularly. On a given row $n$ (i.e. for $u=u_n$), circumference is related to stitch width via
$$
(\text{Stitch width})\cdot(\text{Row $n$ stitch count}) = 2\pi r\sqrt{1-\dot{r}^2}.
$$