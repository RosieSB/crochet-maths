...
- Circular section: outer $(a+p)(\theta+\pi)=${eval}`float(np.round((a+p)*(theta+np.pi),dp))` , inner $(a+p)(\theta+\pi)=${eval}`float(np.round((a-p)*(theta+np.pi),dp))` 
- Straight section: $\Vert Q-P\Vert=${eval}`floar(np.round(magPQ,dp))`
