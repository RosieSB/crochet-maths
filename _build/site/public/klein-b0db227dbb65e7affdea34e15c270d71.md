...
A1 = (e/np.cos(phi)-(H+b*theta+magPQ+a*(np.pi+theta))*np.tan(phi))*np.sqrt(1-(np.tan(phi))**2)
A2 = (np.tan(phi))**2
A3 = Q+(d+b*theta)*PQhat[1]+p*PQhat[2]
A4 = -PQhat[1]
...
so $u_1 = $ {eval}`float(np.round(((gamma2const[1]+p*PQhat[2]-r1const*sqrt1mt2)/(-PQhat[1])+(gamma13const+r1const*tanphi-gamma2const[2]+p*PQhat[1])/(-PQhat[2]))/(tanphi*sqrt1mt2/(-PQhat[1])+(1-tanphi**2)/(-PQhat[2])),dp))`.
