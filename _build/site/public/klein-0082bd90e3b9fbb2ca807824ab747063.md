...
Still working with stitch dimension w={eval}`w`, h={eval}`h`, work a curved pipe formed of {eval}`int(st_count[max(row)-1])` {eval}`int(np.round(2*np.pi*p))` stitches using the technique described in [](#pipe). 
