...
- Eqn. B: {eval}`float(np.round(tanphi*sqrt1mt2/(-PQhat[1]),dp))` $u_1-$  {eval}`float(np.round(,dp))`$u_2 = $ {eval}`float(np.round((gamma2const[1]+p*PQhat[2]-r1const*sqrt1mt2)/(-PQhat[1]),dp))`

- Eqn. C: {eval}`float(np.round(1-tanphi**2,dp))`$u_1 +$ {eval}`float(np.round(-PQhat[2],dp))`$u_2 = $  {eval}`float(np.round(gamma13const+r1const*tanphi-gamma2const[2]+p*PQhat[1],dp))` 
