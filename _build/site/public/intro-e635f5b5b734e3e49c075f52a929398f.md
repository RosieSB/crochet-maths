...
[](#fig:flat-crochet) contains no increases, and an example vertex is labelled $\mathbf{u}$. [](#fig:inc-dec-crochet) has a 2dc increase on row 3 (vertex $\mathbf{v}$) and a 1dc decrease on row 4 (vertex $\mathbf{w}$). The overlayed graphs illustrate the effect of these increases and decreases on the degree of each vertex: $d(\mathbf{u})=4$, $d(\mathbf{v})=6$ and $d(\mathbf{w})=3$. 
