...
Note that voronoidal regions will not all lie strictly inside $T$,if $T$ has an obtuse angle.
...
Example: vertex $v$ with combined voronoidal region $A_v$ and incident angles $\theta_i$.
:::

The *mesh curvature* at $v$ is 
$$
K(v) := \frac{1}{A(v)}\left((2\pi - \sum_i\theta_{i=1}^n)\right)
$$
::::

By the Gauss-Bonnet theorem, $K(v)$ provides a good approximation of the Gauss curvature of the underlying surface.