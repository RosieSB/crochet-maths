...
Basic trig gives
$$
\frac{w}{c+r} = 2\sin\left(\frac{\theta}{2}\right), \; \text{ so } \; c=\frac{w}{2\sin\left(\frac{\theta}{2}\right)}
$$
and,
$$
...
so
$$
\theta_2 = \arcsin\left\frac{w\sin\left(\frac{\pi-\theta}{2})}{d}\right)
$$
...
For these particular measurements, we get $c=$ {eval}`c`, $d=$ {eval}`d`, $\sin(\theta_2) = $ {eval}`sin_theta_2` and $\theta_2=$ {eval}`theta_2`