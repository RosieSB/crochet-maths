...
:label: figs:6dc-12dc-round
:width: 400

So far so flat?
:::

For example, [](#figs:6dc-12dc-round) has 2dc worked into every dc of the first round, for a total of 12dc in round 2. The result appears flat, but as many a crocheter will know, it is rare for a work to stay that way. Trial and error increases produce gently rounded or overly wrinkly work, and though these effects can balance out, often the result is locally mishapen, detracting from the work's aesthetic appeal. 
...
Using the overlaid graph of [](#figs:6dc-12dc-round) and formula [](#eq:mesh-curvature), the central vertex has curvature
$$
...
In these low stitch count cases, a lot hangs on the proportions of the stitches themselves, and the fact they are not rectangles. More to the point, the space a single stitch occupies is different depending on how it is applied, since each stitch necessarily overlaps with other stitches.
...
Moderately careful measurement suggests that for work from [](#fig:labelled-stitch) specifically, $a=${eval}`a`, $b=${eval}`b` and $h=${eval}`h`. Substituting these into [](#eq:theta-n) gives $n=${eval}`n0`, which we round to $n=${eval}`n`. The centre hole radius is $r=${eval}`float(np.round(r,dp))`, and $\theta =${eval}`float(np.round(theta,dp))`. 
...
On the other hand, for [](#figs:6dc-12dc-round), the central vertex $v_0$ has angle defect $2\pi - 6\theta = ${eval}`float(np.round(2*np.pi-6*theta,dp))` radians (positive curvature).