...
How would one prove this? Well, a torus is itself a pipe surface of a circle with radius $R$. Indeed, using [](#eq:pipe-param) with $\gamma(u) = (\cos(u),\sin(u),0)$, we have

\begin{align*}
\mathbf{x}(u,v) &= \gamma(u) + r\left( \mathbf{N}\cos(v)+\mathbf{T}\sin(v) \right) \\
&= (\cos(u),\sin(u),0) + r\Big( (-\cos(u),-\sin(u),0)\cdot\cos(v) \\ 
&\hspace{10em} +(-\sin(u),\cos(u),0)\cdot\sin(v) \Big)
\end{align*}