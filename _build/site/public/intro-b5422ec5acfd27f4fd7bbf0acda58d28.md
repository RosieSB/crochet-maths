...
So, stitch count for round 2 is approximately {eval}`n`$\times$ {eval}`float(theta/theta_2+1)` $\approx$ {eval}`round(n*(theta/theta_2+1))`.
...
$$
For the calculated values of $r =$ {eval}`np.round(r,dp)`, $c =${eval}`c` and $w$ ={eval}`w`, this gives 
:::