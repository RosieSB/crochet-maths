...
$u_1\in[d+b\theta+\Vert\overrightarrow{PQ}\Vert+a(\pi+\theta)+c\sin(\phi), b\theta+\Vert\overrightarrow{PQ}\Vert+a(\pi+\theta)+H-e\sin(\phi)]$, $u_2\in[d+b\theta, d+b\theta+\Vert\overrightarrow{PQ}\Vert]$, $v_1,v_2\in[0,2\pi]$, 
...
$\mathbf{T}_2 = (0,-\hat{\overrightarrow{PQ}}_2,-\hat{\overrightarrow{PQ}}_3)$, $\mathbf{N}_2 = (0,\hat{\overrightarrow{PQ}}_3,-\hat{\overrightarrow{PQ}}_2)$, $\mathbf{B}_2 = (1,0,0)$.
...
(\gamma_1(u_1))_3+r_1\tan\phi
\end{array}\right) \\
&= \left(\begin{array}{c}
p\sin(v_2)\\
(\gamma_2(u_2))_2+p\hat{\overrightarrow{PQ}}_3 \\
(\gamma_2(u_2))_3-p\hat{\overrightarrow{PQ}}_2
...
r_1(u_1)\sqrt{1-\tan^2\phi}\sin(v_1) &=& p\sin(v_2) &\text{ (A)}\\
r_1(u_1)\sqrt{1-\tan^2\phi}\cos(v_1) &=& (\gamma_2(u_2))_2+p\hat{\overrightarrow{PQ}}_3\cos(v_2) &\text{ (B)}\\
(\gamma_1(u_1))_3+r_1\tan\phi &=& (\gamma_2(u_2))_3-p\hat{\overrightarrow{PQ}}_2\cos(v_2) &\text{ (C)}
\end{array}\right.
$$
...
:tags: remove-input
tanphi = np.tan(phi)
r1const = e/np.cos(phi)-(H+b*theta+magPQ+a*(np.pi+theta))*tanphi
sqrt1mt2 = np.sqrt(1-tanphi**2)
gamma13const = H+d+b*theta+magPQ+a*(np.pi+theta)

gamma2const = Q+(d+b*theta)*PQhat
N2 = (0,PQhat[2],-PQhat[1])
:::
...
- Eqn. A: $\big($ {eval}`float(np.round(r1const*sqrt1mt2,dp))` $+$ {eval}`float(np.round(tanphi*sqrt1mt2,dp))` $u_1\big)\sin(v_1) = $ {eval}`float(np.round(p,dp))` $\sin(v_2)$

- Eqn. B: $\big($ {eval}`float(np.round(r1const*sqrt1mt2,dp))` $+$ {eval}`float(np.round(tanphi*sqrt1mt2,dp))` $u_1\big)\cos(v_1) = $ {eval}`float(np.round(gamma2const[1],dp))` $+$ {eval}`float(np.round(-PQhat[1],dp))`$u_2-$ {eval}`float(np.round(-p*PQhat[2],dp))` $\cos(v_2)$

- Eqn. C: {eval}`float(np.round(gamma13const+r1const*tanphi,dp))` $-$ {eval}`float(np.round(1-tanphi**2,dp))`$u_1 = $ {eval}`float(np.round(gamma2const[2],dp))` $+$ {eval}`float(np.round(-PQhat[2],dp))`$u_2+$ {eval}`float(np.round(-p*PQhat[1],dp))` $\cos(v_2)$


Let's first look for solutions in the $y$-$z$ plane, so when $\sin(v_1)=\sin(v_2)=0$, $\cos(v_1)=1$ and $\cos(v_2)=\pm 1$.

When $\cos(v_2)=1$:

- Eqn. B: {eval}`float(np.round(r1const*sqrt1mt2,dp))` $+$ {eval}`float(np.round(tanphi*sqrt1mt2,dp))` $u_1 = $ {eval}`float(np.round(gamma2const[1],dp))` $+$ {eval}`float(np.round(-PQhat[1],dp))`$u_2-$ {eval}`float(np.round(-p*PQhat[2],dp))`

- Eqn. C: {eval}`float(np.round(gamma13const+r1const*tanphi,dp))` $-$ {eval}`float(np.round(1-tanphi**2,dp))`$u_1 = $ {eval}`float(np.round(gamma2const[2],dp))` $+$ {eval}`float(np.round(-PQhat[2],dp))`$u_2+$ {eval}`float(np.round(-p*PQhat[1],dp))`


When $\cos(v_2)=-1$:

- Eqn. B: {eval}`float(np.round(r1const*sqrt1mt2,dp))` $+$ {eval}`float(np.round(tanphi*sqrt1mt2,dp))` $u_1 = $ {eval}`float(np.round(gamma2const[1]-p*PQhat[2],dp))` $+$ {eval}`float(np.round(-PQhat[1],dp))`$u_2$

- Eqn. C: {eval}`float(np.round(gamma13const+r1const*tanphi,dp))` $-$ {eval}`float(np.round(1-tanphi**2,dp))`$u_1 = $ {eval}`float(np.round(gamma2const[2]+p*PQhat[1],dp))` $+$ {eval}`float(np.round(-PQhat[2],dp))`$u_2$
