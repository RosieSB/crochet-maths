...
&= \left(\begin{array}{c}
p\sin(v_2)\\
(\gamma_2(u_2))_1+p\hat{\overrightarrow{PQ}}_2\cos(v_2) \\
(\gamma_2(u_2))_2-p\hat{\overrightarrow{PQ}}_1\cos(v_2)
...
To get the range of rows affected, we first focus on the case where $\sin(v_1)=\sin(v_2)=0$ and $\cos(v_1)=\cos(v_2)=\pm 1$.
...
#u1 = (C3/C1)*u2 + (C4-C2)/C1 = (C3/C1)*( (B1/B3)*u1 + (B2-B4)/B3 ) + (C4-C2)/C1
#= (C3/C1)*(B1/B3)*u1 + (C3/C1)*(B2-B4)/B3 + (C4-C2)/C1 
u1 = ( (C3/C1)*(B2-B4)/B3 + (C4-C2)/C1 )/(1 - (C3/C1)*(B1/B3))
print('When v1 = ',v1,'and v2 = ',v2,', u1 = ',u1)
...
$$
\left\{\begin{array}{rclr}
B_2+B_1u_1 &=& B_4+B_3u_2 &\text{ (A)}\\
C_2+C_1u_1 &=& C_4+C_3u_2 &\text{ (B)}
\end{array}\right.
$$

where $B_1=\tan\phi\sqrt{1-\tan^2\phi}$,
$$
B_2 = \big(e\sec\phi-(H+b\theta+\Vert\overrightarrow{PQ}\Vert+a(\pi+\theta))\tan\phi\big)\sqrt{1-\tan^2\phi}
$$

$$
B_4 = Q_1+ (d+b\theta)\hat{\overrightarrow{PQ}}_1+p\hat{\overrightarrow{PQ}}_2, \hspace{1em} B_3=-\hat{\overrightarrow{PQ}}_1
$$

$$
C_2 = H+d+b\theta+\Vert\overrightarrow{PQ}\Vert+a(\pi+\theta)+e\sec\phi\tan\phi-(H+b\theta+\Vert\overrightarrow{PQ}\Vert+a(\pi+\theta))\tan^2\phi,\hspace{1em} C_1=\tan^2\phi-1
$$

$$
C_4 = Q_2+ (d+b\theta)\hat{\overrightarrow{PQ}}_2-p\hat{\overrightarrow{PQ}}_1, \hspace{1em}C_3=-\hat{\overrightarrow{PQ}}_2
$$
:::{code-cell} python
:tags: remove-input
...
#eqn A: B2+B1u1 = B4+B3u2
B2 = r1const*np.sqrt(1-(np.tan(phi))**2)
B1 = np.tan(phi)*np.sqrt(1-(np.tan(phi))**2)
B4 = Q[1]+(d+b*theta)*PQhat[1]+p*PQhat[2]
B3 = -PQhat[1]

print('Eqn B: ',B1,'u1 + ',B2,' = ',B3,'u2 + ',B4)

#eqn B: C2+C1u1 = C4+C3u2
C2 = H+d+b*theta+magPQ+a*(np.pi+theta)+r1const*np.tan(phi)
C1 = (np.tan(phi))**2-1
C4 = Q[2] + (d+b*theta)*PQhat[2]-p*PQhat[1]
C3 = -PQhat[2]

print('Eqn C: ',C1,'u1 + ',C2,' = ',C3,'u2 + ',C4)

print('\nFrom C: u2 = ',(C2-C4)/C3,' + ',C1/C3,'u1')
print('So, \n',B2,' + ',B1,'u1 = ',B4 + B3*(C2-C4)/C3,' + ',B3*C1/C3,'u1')
print(B1-(B3*C1/C3),'u1 = ',B4+B3*(C2-C4)/C3-B2)
print('u1 = ',(B4+B3*(C2-C4)/C3-B2)/(B1-(B3*C1/C3)))
