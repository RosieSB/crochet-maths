...
:::{admonition} Example
For the pattern example in [](#patt_eg), the parameter values were H={eval}`float(np.round(H,dp))`, a={eval}`a`, b={eval}`b`, d={eval}`float(np.round(d,dp))`, and $\theta=${eval}`theta`. So $P=$(0,{eval}`P[1]`,{eval}`P[2]`), $Q=${eval}`Q`, and $\Vert\overrightarrow{PQ}\Vert=${eval}`magPQ`.
:::
