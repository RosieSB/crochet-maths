...
\begin{align*}
\mathbf{T}(u+\delta u) = \sum_{j=0}^\infty \frac{(\delta u)^j}{j!}\mathbf{T}^{(j)}(u) &= \mathbf{T}(u)\left(1-(\kappa\cdot\delta u)^2+\ldots\right) + \mathbf{N}(u)\left(\kappa\cdot\delta u-\frac{1}{3!}(\kappa\cdot\delta u)^3+\ldots\right) \\
