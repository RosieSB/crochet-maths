...
ax.plot_surface(x4,y4,z4, cmap='viridis',  alpha = .5)
...
#Row count shorthand (for readability)
rc = np.concatenate([rc,rc[7]+1,max(row)],axis=None)
rc = [int(i) for i in rc]

#Graphic
fig, ax = plt.subplots(1, 1, figsize=(4, 6), subplot_kw={'projection': '3d'})

ax.plot_surface(x5,y5,z5, cmap='viridis',  alpha = .5)
...
i = np.argmin(abs(body[:,2]-u1a))+1
row_a = int(body[i,0])
st_count_a = int(body[i,1])

j = np.argmin(abs(body[:,2]-u1b))-1
row_b = int(body[j,0])
...
#### Self-intersection
Goal: By omitting stitches, create appropriately sized hole on the side closest to the handle.

:::{code-cell} python
:tags: hide-input

si_data = np.zeros((j+1-i,4))
instructions = ['strings']*(j+1-i)

for k in range(i,j+1):
    row = int(body[k,0])
    st_count = int(body[k,1])
    increase = int(body[k,1]-body[k-1,1])
    u1 = body[k,2]
    #print('Row ', row, ':', st_count, '-(?) st omitting (?) st from previous row. ')    
    #print('u =',u1)

    #Eqn A: A1*np.sin(v1) = A2*np.sin(v2)
    #( d*np.tan(phi) + e/np.cos(phi)-(H-c*np.sin(phi)-u1+t[5])*np.tan(phi) )*np.sqrt(1-(np.tan(phi))**2)*np.sin(v1) = p*np.sin(v2)) 
    A1 = ( d*np.tan(phi) + e/np.cos(phi)-(H-c*np.sin(phi)-u1+t[5])*np.tan(phi) )*np.sqrt(1 - (np.tan(phi))**2)
    A2 = p
    ...
    #( d*np.tan(phi) + e/np.cos(phi)-(H-c*np.sin(phi)-u1+t[5])*np.tan(phi) )*np.sqrt(1-(np.tan(phi))**2)*np.cos(v1) 
    #= u2*( -PQhat[1] ) + t[2]*PQhat[1] + Q[1] + p*PQhat[2]*np.cos(v2)
    B1 =  ( d*np.tan(phi) + e/np.cos(phi)-(H-c*np.sin(phi)-u1+t[5])*np.tan(phi) )*np.sqrt(1-(np.tan(phi))**2)
    B2 = -PQhat[1]
    B3 = t[2]*PQhat[1] + Q[1] 
    B4 = p*PQhat[2]
    #B1*np.cos(v1) = B2u2+B3+B4*np.cos(v2)
    #-B3+B1*np.cos(v1) = B2u2+B4*np.cos(v2)
    #-B3/B2+(B1/B2)*np.cos(v1) = u2+(B4/B2)*np.cos(v2)
    #print(-B3/B2,'+',B1/B2,'cos(v1) = u2 +',B4/B2,'cos(v2)')

    #Eqn C: C1 = C2u2+C3+C4*np.cos(v2)
    #H-c*np.sin(phi)-u1+t[5] + ( d*np.tan(phi) + e/np.cos(phi)-(H-c*np.sin(phi)-u1+t[5])*np.tan(phi))*np.tan(phi)
    #= u2*( -PQhat[2] ) + t[2]*PQhat[2] + Q[2] - p*PQhat[1]*np.cos(v2)
    C1 = H-c*np.sin(phi)-u1+t[5] + ( d*np.tan(phi) + e/np.cos(phi) - (H-c*np.sin(phi)-u1+t[5])*np.tan(phi) )*np.tan(phi)
    C2 = -PQhat[2]
    C3 = t[2]*PQhat[2] + Q[2] 
    C4 = -p*PQhat[1]
    #C1 = C2u2+C3+C4*np.cos(v2)
    #C1-C3 = C2u2+C4*np.cos(v2)
    #(C1-C3)/C2 = u2+(C4/C2)*np.cos(v2)
    #print((C1-C3)/C2,' = u2 + ',C4/C2,'cos(v2)')

    #(C1-C3)/C2 + B3/B2 - (B1/B2)*np.cos(v1))= (C4/C2)*np.cos(v2) - (B4/B2)*np.cos(v2)
    ...
    #D1**2+2*D1*D2*cos(v1)+D1**2*cos(v1) = D3**2*cos²(v1)
    #D1**2+2*D1*D2*cos(v1)+D1**2*cos(v1) = D3**2*( (A1/A2)**2*cos²(v1) - ((A1/A2)**2-1)) )
    ...
    if -1<=cosv1_a<=1:
        ...
        if -1<=cosv1_b<=1:
            ...
            #print('Angle gap = dv = ',2*v1a,' or ',2*v1b,'?')
            st_omit = '?'
        else:
            #print('v1 = ±',v1a,' = ±',v1a/np.pi,'π') 
            #print('Angle gap = dv = ',2*v1a)
            #ds=sqrt(G)dv
            G = body[k,5]
            ds = np.sqrt(G)*(2*v1a)
            st_omit = round(ds/w)
            #print('G = ',G,', ds = sqrt(G)dv = ',ds, ' \nStitches to omit: ', st_omit)
    else:
        if -1<=cosv1_b<=1:
            v1b = np.arccos(cosv1_b)
            #print('v1 = ±',v1b,' = ±',v1b/np.pi,'π')
            #print('Angle gap = dv = ',2*v1b)
            #ds=sqrt(G)dv
            G = body[k,5]
            ds = np.sqrt(G)*(2*v1b)
            st_omit = round(ds/w)
    ...
    #print('\n')
    st_count = str(st_count)+"-"+str(st_omit)
    si_data[k-i] = [row,st_omit,increase, st_count]
    
    #instructions[k-i] = '**Row {eval}`int(si_data[k-i,0])`:** Omit {eval}`int(si_data[k-1,1])` stitches from the previous row, placing the gap so as to line up with the handle as it passes through the main body section. Work the rest of the round as previous, increasing {eval}`int(si_data[k-i,2])` evenly throughout **({eval}`int(si_data[k-i,3])`-{eval}`int(si_data[k-i,1])` sts)**.'

si_table = tabulate(si_data[2:], headers=['Row','Stitches to omit','Increases','Stitch count (includes missing stitches)'],tablefmt='html')
:::

**Row {eval}`int(si_data[0,0])`:** Omit {eval}`int(si_data[0,1])` stitches from the previous row, placing the gap so as to line up with the handle as it passes through the main body section. Work the rest of the round as usual, increasing {eval}`int(si_data[0,2])` evenly throughout **({eval}`int(si_data[0,3])`-{eval}`int(si_data[0,1])` sts)**.

**Row {eval}`int(si_data[1,0])`:** Leave a gap {eval}`int(si_data[1,1])` stitches wide, aligned and centred relative to the handle and gap from the previous round. Work the rest of the round as usual, increasing {eval}`int(si_data[1,2])` evenly throughout **({eval}`int(si_data[1,3])`-{eval}`int(si_data[1,1])` sts)**.

**Rows {eval}`int(si_data[2,0])`-{eval}`int(max(si_data[:,0]))`:** Use the table below to continue working in this way, leaving a gap of specified width and working increases evenly around each row.

:::{code-cell}
:tags: hide-input
:label: self-intersection-tables
...
To get the range of rows affected, we first focus on the case where $\sin(v_1)=\sin(v_2)=0$ and $\cos(v_1)=\cos(v_2)=\pm1$.
