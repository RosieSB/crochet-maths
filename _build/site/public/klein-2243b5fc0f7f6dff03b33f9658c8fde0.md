...
1. Make a foundation chain of {eval}`int(base7[0,1])` stitches, and join into a ring. 
2. {eval}`int(base7[0,1])` dc into foundation chain.
3. Work the following stitch counts evenly throughout each round to form the inside of the base/very bottom of the handle:
:::{code-cell} python
:label: handle-table
:tags: remove-input
...
:::
5. Work {eval}`int(base7[0,1])` dc into the other side of the foundation chain. 
6. 
...
table = tabulate(data,headers=['Row','Stitch count'],tablefmt='html')
