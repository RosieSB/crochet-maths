...
- Eqn. C: $\big($ {eval}`float(np.round(r1const*sqrt1mt2,dp))` $+$ {eval}`float(np.round(tanphi*sqrt1mt2,dp))` $u_1\big)\cos(v_1) = $ {eval}`float(np.round(gamma2const[2],dp))` $+$ {eval}`float(np.round(-PQhat[2],dp))`$u_2+$ {eval}`float(np.round(-p*PQhat[1],dp))` $\cos(v_2)$
