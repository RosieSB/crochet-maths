...
Repeat the following two rows {eval}`int(np.round((np.pi+theta)/turning_angle_a))` times:
 ...
 - Work a short row of {eval}`dual_st_count_a[1]` stitches, and placed on the **opposite** side to the previous short rows, so that the pipe turns in the opposite direction.
