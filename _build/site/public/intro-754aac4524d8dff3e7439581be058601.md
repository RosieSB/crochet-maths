...
[](#fig:flat-crochet) contains no increases, and an example vertex is labelled $u$. [](#fig:inc-dec-crochet) has a 2dc increase on row 3 (vertex $v$) and a 1dc decrease on row 4 (vertex $w$). The overlayed graphs illustrate the effect of these increases and decreases on the degree of each vertex: $d(u)=4$, $d(v)=6$ and $d(w)=3$. 
