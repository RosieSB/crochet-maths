...
::::
:::::
Both functions are composed of straight and circular segments. The directrix [](#fig:directrix) is parametrised by arc length, and so is unit speed. This simplifies the canal surface parametrisation [](#eq:canal-param) somewhat:
:::{math}
:enumerated: true
...
ObserveNote that the radius function $r$ ([](#fig:radius-fn)) has an unbounded derivative near $z=d$, and so the surface cannot be realised as a canal surface in this section. 

The directrix, [](#fig:directrix), is The directrix,  [](#fig:directrix), is parametrised by arc length.See below for more specific formulae and dependencies between the parameters.
