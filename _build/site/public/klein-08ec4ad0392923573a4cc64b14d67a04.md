...
- Eqn. C: {eval}`float(np.round(gamma13const+r1const*tanphi-gamma2const[2]+p*PQhat[1],dp))` $ = ${eval}`float(np.round(1-tanphi**2,dp))`$u_1 +$ {eval}`float(np.round(-PQhat[2],dp))`$u_2$
