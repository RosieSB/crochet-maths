...
Linear section: we have end points $X=((p-2d)\cos\phi,(p-2d)\sin\phi)$, $Y=(p+c-c\cos\phi,h-c\sin\phi)$, so line through $X$ and $Y$ has equation $z=(-\cot\phi)r +k$, where
$$
...
$$
h\sin\phi+(c+p)\cos\phi = c+p-2d,
$$
or in other words,
:::{math}
:enumerated: true
:label:pphi-constraint
d= \frac{(c+p)(1-\cos\phi) -h\sin\phi}{2}
:::
...
- $r_2(u)=(p-2d)\sec\phi-u\tan\phi$, $\dot r_2(u)=-\tan\phi$, for $u\in[(p-2d)\sin\phi,h-c\sin\phi]$.
...
P = np.array([0,c*(1+np.cos(theta)),h-c*np.sin(theta)])
Q = np.array([0,b*(1-np.cos(theta)),b*np.sin(theta)])
PQ = Q-P
magPQ = np.sqrt(PQ[1]**2+PQ[2]**2)
PQhat = PQ/magPQ
...
#Radius function
r = np.sqrt((p-2*d)**2-u**2)
rdot = -u/np.sqrt((p-2*d)**2-u**2)
...
#Radius function
r = (p-2*d)/np.cos(phi)-u*np.tan(phi)
rdot = -np.tan(phi)
