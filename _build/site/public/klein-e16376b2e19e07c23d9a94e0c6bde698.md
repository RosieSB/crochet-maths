...
Still working with stitch dimension w={eval}`w`, h={eval}`h`, work a curved pipe formed of {eval}`int(st_count[N-1])` stitches using the technique described in [](#pipe). 
