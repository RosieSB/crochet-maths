...
**Rows 1–{eval}`max(row7)`:**  Work the following stitch counts evenly throughout each round to form the inside of the base/very bottom of the handle:
:::::{grid} 2 2 2 2
::::{grid-item}
:::{code-cell} python
:label: handle-table
...
**Rows {eval}`max(row7)+1`–{eval}`max(row7)+2*int(np.round(theta/turning_angle_b))`:**
