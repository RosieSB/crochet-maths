...
n = int(round(2*np.pi/theta,dp))

r = (b*h)/(a-b)
:::

Moderately careful measurement suggests that for work from [](#fig:labelled-stitch) specifically, $a=${eval}`a`, $b=${eval}`b` and $h=${eval}`h`. Substituting these into [](#eq:theta-n) gives $\theta =${eval}`float(np.round(theta,dp))` and $n=${eval}`n`. The centre hole radius is $r=${eval}`float(np.round(r,dp))`. 
...
On the otherhand, for [](#figs:6dc-12dc-round), the central vertex $v_0$ has angle defect $2\pi - 6\theta = ${eval}`float(np.round(2*np.pi-6*theta,dp))` radians (positive curvature).