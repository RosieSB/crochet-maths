...
I would like to make one more pattern, with the correct stitch dimensions w={eval}`w`, h={eval}`h`, and larger choices for R and r. In particular, I would like one twice as big. 
...
fig = plt.figure(figsize = (8,2),label=' ',layout='tight')
ax = plt.axes()
area = 3**2
