...
Edu^2+2Fdudv+Gdv^2
$$
associated with the parametrised surface. Specifically, $E:=\mathbf{x}_u^2$ determines row count, and $G:=\mathbf{x}_v^2$ determines stitch count per row. In fact, $F=0$ for each parametrisation (Clairaut surface). See [](#towards-crochetability) for more details.
