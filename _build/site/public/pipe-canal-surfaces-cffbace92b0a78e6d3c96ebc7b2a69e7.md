...
kappa_v = float(np.round(1/r,3))
kappa_u_max = float(np.round(1/(R+r),3))
kappa_u_min = float(np.round(-1/(R-r),3))
...
Parameter values: $R=${eval}`R`, $r=${eval}`r`, $w=${eval}`w` and $h=${eval}`h`. The curvature of the directrix circle is $\kappa=\frac{1}{R}=${eval}`kappa`. Principal curvatures of the produced surface are $\kappa_v=\frac{1}{r}=${eval}`kappa_v` and $\kappa_u$, which varies between $-\frac{1}{R-r}=${eval}`kappa_u_min` inside the bend, and $\frac{1}{R+r}=${eval}`kappa_u_max` on the outermost point of the bend. 
...
Curvature: 
- The curvature of the directrix circle is $\kappa=\frac{1}{R}=${eval}`kappa`. 

- Principal curvatures of the pipe are $\kappa_v=\frac{1}{r}=${eval}`kappa_v` and $\kappa_u$, which varies between $-\frac{1}{R-r}=${eval}`kappa_u_min` inside the bend, and $\frac{1}{R+r}=${eval}`kappa_u_max` on the outermost point of the bend. 
