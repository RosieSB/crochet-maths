...
- The parameter values were H = {eval}`float(np.round(H,dp))`, a = {eval}`a`, b = {eval}`b`, d = {eval}`float(np.round(d,dp))`, and θ = {eval}`theta`. 
- P = (0,{eval}`float(np.round(P[1],dp))`,{eval}`float(np.round(P[2],dp))`), 
- Q = (0,{eval}`float(np.round(Q[1],dp))`,{eval}`float(np.round(Q[2],dp))`),
- $\Vert\overrightarrow{PQ}\Vert=${eval}`float(np.round(magPQ,dp))`.
:::
