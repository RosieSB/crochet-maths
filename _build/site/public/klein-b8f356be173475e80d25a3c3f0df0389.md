...
$$
\gamma_1 = \left(0,0,H-c\sin(\phi)-u+d+b\theta+\Vert\overrightarrow{PQ}\Vert+a(\pi+\theta)+c\sin(\phi))\right)
$$

$$
r_1 = d\tan(\phi) + e\sec(\phi)-(H-c\sin(\phi)-u+d+b\theta+\Vert\overrightarrow{PQ}\Vert+a(\pi+\theta)+c\sin(\phi))\tan(\phi)
$$
...
#Bottle canal 5
u = [d+b*theta+magPQ+a*(np.pi+theta)+c*np.sin(phi), b*theta+magPQ+a*(np.pi+theta)+H-e*np.sin(phi)]
r5 = d*np.tan(phi) + e/np.cos(phi)-(H-c*np.sin(phi)-u+t[5])*np.tan(phi)
r5dot = np.tan(phi)*np.ones(len(u))
r5ddot = np.zeros(len(u))
gamma5 = (0*u,0*u,H-c*np.sin(phi)-u+t[5])
