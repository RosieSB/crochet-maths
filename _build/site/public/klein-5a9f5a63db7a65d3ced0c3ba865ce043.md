...
usplit = [np.linspace(-np.pi, -np.pi/2, n),   #np.linspace(t[0], t[1], n),
          ...
          np.linspace(-np.pi/2, 0, n)   #np.linspace(t[7], t[8], n)
          ]


# Set up a figure and axes
fig, surf = plt.subplots(1, 1, figsize=(8, 12), subplot_kw={'projection': '3d'})
#fig2, directrix = plt.subplots(1, 1, figsize=(8, 12))
...
r = p
rdot = 0
gamma6 = (0*u, b+b*np.cos(np.pi-(u-t[1])/b), d+b*np.sin(np.pi-(u-t[1])/b))
...
#Handle 5
u = usplit[2]
gamma5 = (0*u,((u-t[2])/magPQ)*P[1] + (1-(u-t[2])/magPQ)*Q[1],((u-t[2])/magPQ)*P[2] + (1-(u-t[2])/magPQ)*Q[2])
T = (0,-PQhat[1],-PQhat[2])
N = (0,PQhat[2],-PQhat[1])
v = np.linspace(0,2*np.pi,100)
u, v = np.meshgrid(u, v)
...
#Handle 4
u = usplit[3]
gamma4 = ( 0*u, a*(1+np.cos( (u-t[3])/a-theta )), H+a*np.sin( (u-t[3])/a-theta ) )
...
r3ddot = -c**2*np.power(c**2-(-u+t[4])**2,-1.5)
gamma3 = (0*u,0*u,H-u+t[4])
T = (0,0,-1)
N = (0,1,0)
B = (1,0,0)
v = np.linspace(0,2*np.pi,100)
u, v = np.meshgrid(u, v)
x3 = gamma3[0] - r3*r3dot*T[0] + r3*np.sqrt(1-(r3dot)**2)*(N[0]*np.cos(v)+B[0]*np.sin(v)) 
y3 = gamma3[1] - r3*r3dot*T[1] + r3*np.sqrt(1-(r3dot)**2)*(N[1]*np.cos(v)+B[1]*np.sin(v)) 
z3 = gamma3[2] - r3*r3dot*T[2] + r3*np.sqrt(1-(r3dot)**2)*(N[2]*np.cos(v)+B[2]*np.sin(v))
...
#Bottle canal 2
u = usplit[5]
r2 = d*np.tan(phi) + e/np.cos(phi)-(H-c*np.sin(phi)-u+t[5])*np.tan(phi)
r2dot = np.tan(phi)*np.ones(len(u))
r2ddot = np.zeros(len(u))
gamma2 = (0*u,0*u,H-c*np.sin(phi)-u+t[5])
v = np.linspace(0,2*np.pi,100)
u, v = np.meshgrid(u, v)
x2 = gamma2[0] - r2*r2dot*T[0] + r2*np.sqrt(1-(r2dot)**2)*(N[0]*np.cos(v)+B[0]*np.sin(v)) 
y2 = gamma2[1] - r2*r2dot*T[1] + r2*np.sqrt(1-(r2dot)**2)*(N[1]*np.cos(v)+B[1]*np.sin(v)) 
z2 = gamma2[2] - r2*r2dot*T[2] + r2*np.sqrt(1-(r2dot)**2)*(N[2]*np.cos(v)+B[2]*np.sin(v))
...
#Bottle canal 1
u = usplit[6]
r1 = np.sqrt(e**2-(e*np.sin(phi)-u+t[6])**2)
r1dot = (e*np.sin(phi)-u+t[6])/np.sqrt(e**2-(e*np.sin(phi)-u+t[6])**2)
r1ddot = -e**2*np.power((e**2-(e*np.sin(phi)-u+t[6])**2),-1.5)
gamma1 = (0*u,0*u,e*np.sin(phi)-u+t[6]+d)
v = np.linspace(0,2*np.pi,100)
u, v = np.meshgrid(u, v)
...
:label: handle
:tags: remove-input
fig, ax = plt.subplots(1, 1, figsize=(8, 12), subplot_kw={'projection': '3d'})
...
row7 = [int(i/h) for i in s7]

#circumference
C7 = 2*np.pi*np.sqrt(G7)
st_count7 = np.round(C7/w)

base7 = np.transpose([row7,
                     st_count7
                     ])
...
#### Inner base
**Row 0:** Make a foundation chain of {eval}`int(base7[0,1])` stitches, and join into a ring. 

**Rows 1–{eval}`max(row)`:**  Work the following stitch counts evenly throughout each round to form the inside of the base/very bottom of the handle:
:::{code-cell} python
:label: handle-table
:tags: remove-input
table = tabulate(base7,headers=['Row','Stitch count'],tablefmt='html')
display(table)
:::

**Lower curved section.**
4. Turn work upside down, to work the next round into the other side of the foundation chain.
:::{tip} Important note!
...
:::
5. Work {eval}`int(base7[0,1])` dc around. 
6. Work a **short row** of {eval}`dual_st_count_b[1]` stitches into the next round.
7. Repeat Steps 5 and 6 {eval}`int(np.round(theta/turning_angle_b))` times. Pipe should have turned approximately {eval}`theta` radians.
**Straight section.**
8. {eval}`dual_st_count_b[0]` dc in each st around, {eval}`int(np.round(magPQ/h))` times.
**Upper curved section.** 
9. Work {eval}``dual_st_count_a[0]` dc around. 
10. Work another short row, this time of {eval}`dual_st_count_a[1]` stitches, and placed on the **opposite** side to the previous short rows, so that the pipe turns in the opposite direction.
11. Repeat Steps 5 and 6 {eval}`int(np.round((np.pi+theta)/turning_angle_a))` times. Pipe should have turned approximately {eval}`float(np.round(np.pi+theta,dp))` radians.
...
# Set up a figure and axes
#fig1, surf = plt.subplots(1, 1, figsize=(8, 12), subplot_kw={'projection': '3d'})
#fig2, directrix = plt.subplots(1, 1, figsize=(8, 12))
...
### Base
For $-\pi\leq u\leq 0$ and $0\leq v\leq 2\pi$,
$$
