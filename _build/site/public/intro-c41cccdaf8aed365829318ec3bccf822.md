...
theta = float(np.round(2*np.asin((a-b)/(2*h)),dp))

n = float(np.round(2*np.pi/theta,dp))

r = float(np.round((b*h)/(a-b),dp))
:::

Moderately careful measurement suggests that for work from [](#fig:labelled-stitch) specifically, $a=${eval}`a`, $b=${eval}`b` and $h=${eval}`h`. Substituting these into [](#eq:theta-n) gives $\theta =${eval}`theta` and $n=${eval}`n`. The centre hole radius is $r=${eval}`r`. 