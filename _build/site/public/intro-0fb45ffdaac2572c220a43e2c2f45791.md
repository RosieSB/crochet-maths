...
The two patches are crocheted in UK double crochet (dc), with identical yarn, hook and tension, and with all stitches facing the same way (the crochet version of stockinette). For the overlayed graph, every stitch gets a vertex, and two stitches are considered adjacent if they are next to one another on the same row, or if one stitch is worked into the top of the other. 

[](#fig:flat-crochet) contains no increases, and an example vertex is labelled $u$, while [](#fig:inc-dec-crochet) has a 2dc increase on row 3 (vertex $v$) and a 1dc decrease on row 4 (vertex $w$). The overlayed graphs illustrate the effect of these increases and decreases on the degree of each vertex: $d(u)=4$, $d(v)=6$ and $d(w)=3$. 
