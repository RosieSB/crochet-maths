...
For the top part of the handle, use radii R=a={eval}`a` and r=p={eval}`p`, repeating the rows of [Example 1](#Ex1) until you have turned a total angle of {eval}`np.pi+theta` radians.
