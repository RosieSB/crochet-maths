...
Below is an example, made with parameter values d={eval}`d`, a={eval}`a` and b={eval}`b`. From these we get P=(0,{eval}`float(np.round(P[1],3))`,{eval}`float(np.round(P[2],3))`), Q=(0,{eval}`float(np.round(Q[1],3))`,{eval}`float(np.round(Q[2],3))`) and h={eval}`float(np.round(h,3))`.
...
fig = plt.figure(figsize = (8,7.5), label = ' ')
ax = plt.axes(projection='3d')
