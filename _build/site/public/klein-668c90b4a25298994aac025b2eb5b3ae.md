...
- Eqn. C: $\big($ {eval}`float(np.round(gamma13const+r1const*tanphi,dp))` $-$ {eval}`float(np.round(1-tanphi**2,dp))`$u_1$  = $ {eval}`float(np.round(gamma2const[2],dp))` $+$ {eval}`float(np.round(-PQhat[2],dp))`$u_2+$ {eval}`float(np.round(-p*PQhat[1],dp))` $\cos(v_2)$
