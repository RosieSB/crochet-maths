...
-\tan\phi\sqrt{1-\tan^2\phi}\sin(v)\\[1em]
-\tan\phi\sqrt{1-\tan^2\phi}\cos(v)\\[1em]
1-\tan^2\phi
\end{array}\right),
$$
...
$$
G = \mathbf{x}_v^2 = \big(d\tan\phi+e\sec\phi-u\tan\phi\big)^2\left(1-\tan^2\phi\right)
$$

So for $d+e\sin\phi \leq u \leq h-c\sin\phi$, the line element is

$$
ds^2 = (1-\tan^2\phi)du^2 + \big(d\tan\phi+e\sec\phi-u\tan\phi\big)^2\left(1-\tan^2\phi\right)dv^2.
$$

In particular, $\sqrt{1-\tan^2\phi}du$ should count rows. 

## Main body - circular radius 2
The last section of main body covers $ d+h-c\sin\phi\leq u\leq h$ and
$$
r= p+c-\sqrt{c^2-(u-d-h)^2},\hspace{1em}\dot{r}=\frac{u-d-h}{\sqrt{c^2-(u-d-h)^2}},
$$
with $\gamma$, $\mathbf{T}$, $\mathbf{N}$ and $\mathbf{B}$ unchanged. By [](#eq:body),


$$
\mathbf{x} = \left(r\sqrt{1-\dot{r}^2}\sin(v),r\sqrt{1-\dot{r}^2}\cos(v),u-r\dot{r}\right)
$$

$$
\mathbf{x} = \left(\begin{array}{c}
\left(p+c-\sqrt{c^2-(u-d-h)^2}\right)\sqrt{\frac{x^2-2(u-d-h)^2}{c^2-(u-d-h)^2}}\sin(v)\\[1em]
\left(p+c-\sqrt{c^2-(u-d-h)^2}\right)\sqrt{\frac{x^2-2(u-d-h)^2}{c^2-(u-d-h)^2}}\cos(v)\\[1em]
2u-d-h-\frac{(p+c)(u-d-h)}{\sqrt{c^2-(u-d-h)^2}}
\end{array}\right),
$$