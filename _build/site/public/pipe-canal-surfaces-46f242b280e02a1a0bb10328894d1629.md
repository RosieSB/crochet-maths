...
How would one prove this? Well, a torus is itself a pipe surface of a circle with radius $R$. Indeed, using [](#eq:pipe-param) with $\gamma(u) = (R\cos(u),R\sin(u),0)$, we have 

\begin{align*}
\mathbf{x}(u,v) &= \gamma(u) + r\left( \mathbf{N}\cos(v)+\mathbf{T}\sin(v) \right) \\
&= (R\cos(u),R\sin(u),0) + r\Big( (-\cos(u),-\sin(u),0)\cdot\cos(v) \\ 
&\hspace{12em} +(-\sin(u),\cos(u),0)\cdot\sin(v) \Big) \\
&= (R\cos(u),R\sin(u),0) + \Big( (-r\cos(u)\cos(v),-r\sin(u)\cos(v),0) \\ 
&\hspace{12em} +(-r\sin(u)\sin(v),r\cos(u)\sin(v),0) \Big) \\
\end{align*}