...
On the otherhand, for [](#figs:6dc-12dc-round), the central vertex $v_0$ has angle defect $2\pi - 10\theta = ${eval}`float(np.round(2*np.pi-10*theta,dp))` radians (positive curvature).