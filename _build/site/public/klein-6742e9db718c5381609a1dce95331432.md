...
Beginning with the main bottle and ignoring the base for now, we have unit speed directrix $\gamma(u)=(0,0,u)$ and Frenet-Serret apparatus $\mathbf{T}=(0,0,1)$, $\mathbf{N}=(0,1,0)$, $\mathbf{B}=(1,0,0)$. Moreover,
$$
\begin{aligned}
\mathbf{x}(u,v) &= \gamma-r\dot{r}\mathbf{T} +r\sqrt{1-\dot{r}^2}\Big( \mathbf{N}\cos(v)+\mathbf{B}\sin(v)\Big)
\end{aligned}
...
Lengths. For crochet. We have line element
$$
ds^2 = Edu^2+2Fdudv+Gdv^2=\frac{4\left(e^2-u^2\right)}{e^2-2u^2}du^2+\left(e^2-2u^2\right)dv^2
$$

So row count "should" be (approximately) $\frac{2\sqrt{e^2-u^2}}{\sqrt{e^2-2u^2}}$, for a faithful representation of shape via crochet. Unless I am having a mental blip.

Returning to the parametrisation, and using explicitly $\gamma=(0,0,u)$, $\mathbf{T}=(0,0,1)$, $\mathbf{N}=(0,1,0)$ and $\mathbf{B}=(1,0,0)$ (possibly normals are pointing the wrong way but  it shouldn't matter for now)
...
Suppose we have crocheted $n$ rows, each of stitch height $h$. If the height of the work is $z=2u$ then we'd expect the length of produced fabric to satisfy
$$
