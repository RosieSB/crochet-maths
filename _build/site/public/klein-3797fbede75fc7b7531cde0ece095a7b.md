...
r = p
rdot = 0
gamma6 = (0*u, b+b*np.cos(np.pi-theta-(u-t[1])/b), d+b*np.sin(np.pi-theta-(u-t[1])/b))
T = (0,np.sin(np.pi-theta-(u-t[1])/b),-np.cos(np.pi-theta-(u-t[1])/b))
N = (0,np.cos(np.pi-theta-(u-t[1])/b),np.sin(np.pi-theta-(u-t[1])/b))
B = (1,0,0)
v = np.linspace(0,2*np.pi,100)
u, v = np.meshgrid(u, v)
