...
More generally
\begin{gather*}
\frac{d^{2m}\mathbf{T}}{du^{2m}} = (-1)^m\kappa^{2m}\mathbf{T}, \;\; \frac{d^{2m+1}\mathbf{T}}{du^{2m+1}} = (-1)^m\kappa^{2m+1}\mathbf{N}, \\
\frac{d^{2m}\mathbf{N}}{du^{2m}} = (-1)^m\kappa^{2m}\mathbf{N}, \;\; \frac{d^{2m+1}\mathbf{T}}{du^{2m+1}} = (-1)^m\kappa^{2m+1}\mathbf{T}, 
\end{gather}

Hence for small changes in $u$,
\begin{align*}
\mathbf{T}(u+\delta u) &= \mathbf{T}(u) +\delta u\frac{d\mathbf{T}}{du}+\frac{1}{2!}(\delta u)^2\frac{d^2\mathbf{T}}{du^2} + ... \\
&= \mathbf{T}(u)\left(1-(\kappa\cdot\delta u)^2+\ldots\right) + \mathbf{N}(u)\left(\kappa\delta u-\frac{1}{3!}(\kappa\delta u)^3+\ldots\right) \\
&= \cos(\kappa\delta u)\mathbf{T}(u) + \sin(\kappa\delta u)\mathbf{N}(u) \\ 
&= \cos(\kappa\delta u)(0,1,0) + \sin(\kappa\delta u)(-1,0,0) \\
&= \big(-\sin(\kappa\delta u),\cos(\kappa\delta u),0\big)
\end{align*}
and 
$$
\mathbf{N}(u+\delta u) = (-1,0,0)+\delta u\frac{d\mathbf{N}}{du}+(\delta u)^2\frac{d^2\mathbf{N}}{du^2} + ...
$$
...
$$
\mathbf{x}(u+\delta u,v) = \gamma(u+\delta u) + r\left( \mathbf{N}\cos(v)+\mathbf{B}\sin(v) \right)
$$