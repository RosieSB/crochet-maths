...
- Eqn. B: $\big($ {eval}`float(np.round(r1const*sqrt1mt2,dp))` $+$ {eval}`float(np.round(tanphi*sqrt1mt2,dp))` $u_1\big)\cos(v_1) = $ {eval}`gamma2const[1]` $-$ {eval}`PQhat`$u_2+$ {eval}`float(np.round(p*PQhat[2],dp))` $\cos(v_2)$
