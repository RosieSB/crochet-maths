...
How would one prove this? Well, a torus is itself a pipe surface of a circle with radius $R$. Indeed, using [](#eq:pipe-param) with $\gamma(u) = (R\cos(u/R),R\sin(u/R),0)$, we have

\begin{align*}
\mathbf{x}(u,v) &= \gamma(u) + r\left( \mathbf{N}\cos(v)+\mathbf{T}\sin(v) \right) \\
&= (\cos(u),\sin(u),0) + r\Big( (-\cos(u/R),-\sin(u/R),0)\cdot\cos(v) \\ 
&\hspace{12em} +(-\sin(u/R),\cos(u/R),0)\cdot\sin(v) \Big) \\
&= (\cos(u),\sin(u),0) + r\Big( (-\cos(u),-\sin(u),0)\cdot\cos(v) \\ 
