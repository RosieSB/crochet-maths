...
\begin{aligned}
\mathbf{x}(u,v) = \mathbf{x}(u,v) &= \gamma-r\dot{r}\mathbf{T} +r\sqrt{1-\dot{r}^2}\Big( \mathbf{N}\cos(v)+\mathbf{B}\sin(v)\Big)
\end{aligned}
:::

Observe that the radius function $r$ ([](#fig:radius-fn)) has an unbounded derivative near $z=d$. For this reason, the canal surface parametrisation will not work in this section, and we use a surface of revolution instead. 

See below for more specific formulae and dependencies between the parameters $(H, $a$, $b$, $\theta$, etc.).
